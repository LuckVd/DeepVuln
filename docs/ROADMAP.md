...
|Phase 5|精度深化|可利用性评估增强 + 调用图分析|**→ 当前阶段**|2026-03|
...
|P5-04|攻击面检测模式重构（静态+LLM并行）|P5-03|**→ 当前任务**|
...
|v0.6|精度深化|可利用性评估增强 + 调用图分析|**testing**|2026-03-07|
|v0.7|企业稳定版|高精度、低误报、CI 可用|todo|-|
...
|---|---|
|**阶段**|Phase 5 - 精度深化|
|**当前进度**|P5-03 完成 ✅，P5-04 攻击面检测模式重构启动 🚀|
|**下一步**|P5-04a (模式重构) → P5-04b (结果去重) → P5-04c (CLI集成)|
|**重点模块**|l1_intelligence (attack_surface) / cli|
|**目标**|默认静态+LLM并行检测，结果合并去重|
